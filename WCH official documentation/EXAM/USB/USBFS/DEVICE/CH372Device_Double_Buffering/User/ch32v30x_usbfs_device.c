...
__attribute__ ((aligned(4))) uint8_t USBFS_EP1_Buf[ DEF_USBD_EP1_FS_SIZE*2 ];  //ep1_out(64*2)
__attribute__ ((aligned(4))) uint8_t USBFS_EP2_Buf[ DEF_USBD_EP2_FS_SIZE*2 ];  //ep2_in(64*2)
__attribute__ ((aligned(4))) uint8_t USBFS_EP3_Buf[ DEF_USBD_EP3_FS_SIZE*2 ];  //ep3_out(64*2)
__attribute__ ((aligned(4))) uint8_t USBFS_EP4_Buf[ DEF_USBD_EP4_FS_SIZE*2 ];  //ep4_in(64*2)
...
void USBFS_Device_Endp_Init( void )
{
    ...
    USBFSD->UEP4_1_MOD = USBFS_UEP4_TX_EN|USBFS_UEP1_RX_EN|USBFS_UEP1_BUF_MOD|USBFS_UEP4_BUF_MOD;
    USBFSD->UEP2_3_MOD = USBFS_UEP2_TX_EN|USBFS_UEP3_RX_EN|USBFS_UEP2_BUF_MOD|USBFS_UEP3_BUF_MOD;
    ...
    USBFSD->UEP6_TX_LEN = 64;

    USBFSD->UEP0_TX_CTRL = USBFS_UEP_T_RES_NAK;
    USBFSD->UEP2_TX_CTRL = USBFS_UEP_T_RES_NAK;
    USBFSD->UEP4_TX_CTRL = USBFS_UEP_T_RES_NAK;
    USBFSD->UEP6_TX_CTRL = USBFS_UEP_T_RES_ACK;

    for (size_t i = 0; i < sizeof(USBFS_EP6_Buf); i++)
    {
        USBFS_EP6_Buf[i] = i;
    }
                ...
                switch ( intst & ( USBFS_UIS_TOKEN_MASK | USBFS_UIS_ENDP_MASK ) )
                {
                    ...
                    case USBFS_UIS_TOKEN_IN | DEF_UEP0:
                        ...
                        case ( USBFS_UIS_TOKEN_IN | DEF_UEP2 ):
                            ...
                            if (USBFSD->UEP2_TX_CTRL & USBFS_UEP_T_TOG)
                            {   // DATA0
                                USBFS_Endp_Busy[ DEF_UEP2 ] &= ~0x01;
                            }
                            else
                            {   // DATA1
                                USBFS_Endp_Busy[ DEF_UEP2 ] &= ~0x02;
                            }
                        ...
                        case ( USBFS_UIS_TOKEN_IN | DEF_UEP4 ):
                            ...
                            if (USBFSD->UEP4_TX_CTRL & USBFS_UEP_T_TOG)
                            {   // DATA0
                                USBFS_Endp_Busy[ DEF_UEP4 ] &= ~0x01;
                            }
                            else
                            {   // DATA1
                                USBFS_Endp_Busy[ DEF_UEP4 ] &= ~0x02;
                            }
                ...
                switch ( intst & ( USBFS_UIS_TOKEN_MASK | USBFS_UIS_ENDP_MASK ) )
                {
                    ...
                    case USBFS_UIS_TOKEN_OUT | DEF_UEP1:
                        if ( intst & USBFS_UIS_TOG_OK )
                        {
                            ...
                            if (USBFSD->UEP1_RX_CTRL & USBFS_UEP_R_TOG)
                            {   // DATA0
                                USBFS_Endp_Busy[ DEF_UEP2 ] |= 0x01;
                                for (i = 0; i < len; i++)
                                {
                                    USBFS_EP2_Buf[i] = USBFS_EP1_Buf[i];
                                }
                            }
                            else
                            {   // DATA1
                                ...
                                for (i = 0; i < len; i++)
                                {
                                    USBFS_EP2_Buf[64 + i] = USBFS_EP1_Buf[64 + i];
                                }
                            }
                    ...
                    case USBFS_UIS_TOKEN_OUT | DEF_UEP3:
                        if ( intst & USBFS_UIS_TOG_OK )
                        {
                            ...
                            if (USBFSD->UEP3_RX_CTRL & USBFS_UEP_R_TOG)
                            {   // DATA0
                                USBFS_Endp_Busy[ DEF_UEP4 ] |= 0x01;
                                for (i = 0; i < len; i++)
                                {
                            ...
                            {   // DATA1
                                ...
                                for (i = 0; i < len; i++)
                                {
                                    USBFS_EP4_Buf[64 + i] = ~USBFS_EP3_Buf[64 + i];
                                }
                            }
