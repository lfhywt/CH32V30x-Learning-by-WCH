...
void USBFS_Device_Endp_Init( void )
{
    ...
    USBFSD->UEP3_TX_LEN = DEF_USBD_EP3_FS_SIZE;
                            ...
                            if( ( USBFS_SetupReqType & USB_REQ_TYP_MASK ) != USB_REQ_TYP_STANDARD )
                            {
                                if( ( USBFS_SetupReqType & USB_REQ_TYP_MASK ) == USB_REQ_TYP_CLASS )
                                {
                                    ...
                                    switch( USBFS_SetupReqCode )
                                    {
                                        case UAC_SET_CUR:
                                            /* Entity ID: 5 */
                                            if (USBFS_SetupReqIndex == 0x0500)
                                            {
                                                switch ((USBFS_SetupReqValue >> 8) & 0xFF) 
                                                {
                                                    case UAC_CS_MUTE_CONTROL:
                                                        uac_microphone_unit.feature_unit.mute = USBFS_EP0_Buf[ 0 ];
                                                        USBFS_SetupReqLen = 0;
                                                        break;
                                                    default:
                                                        errflag = 0xFF;
                        ...
                        switch( USBFS_SetupReqCode )
                        {
                            case UAC_GET_CUR:
                                ...
                                if (USBFS_SetupReqIndex == 0x0500)
                                {
                                    ...
                                    {
                                        case UAC_CS_MUTE_CONTROL:
                                            USBFS_EP0_Buf[ 0 ] = uac_microphone_unit.feature_unit.mute;    
                                            break;
                                        default:
                                            errflag = 0xFF;
                                            break;
                ...
                else
                {
                    ...
                    switch( USBFS_SetupReqCode )
                    {
                        ...
                        case USB_SET_INTERFACE:
                            ...
                            UAC_Stream_Receive_Ctrl(USBFS_Interface);
                            break;
